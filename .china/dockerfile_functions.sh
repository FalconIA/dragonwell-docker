...
if [[ "${vm}" == "dragonwell"  && ( "${os}" == "ubi-minimal" || "${os}" == "ubi" ) ]]; then
	cat >> "$1" <<-EOI
...
LABEL name="Alibaba Dragonwell JDK" \\
      ...
      summary="Docker Image for Docker Image for Alibaba Dragonwell JDK with ${vm} and ${os}" \\
...
EOI
elif [ "${os}" == "ubi-minimal" ] || [ "${os}" == "ubi" ]; then
	cat >> "$1" <<-EOI
...
print_java_install_pre() {
	...
	if [ "${vm}" == "dragonwell" ]; then
       ...
       amd64|x86_64) \\
         ...
         BINARY_URL='$(sarray="${shasums}[url-binary-x86_64]"; eval binary_url=\${$sarray}; echo "${binary_url}")'; \\
         ;; \\
EOI
			fi
		done
